...
int main()
{
	...
	while(true) {
		...
		//오른쪽에 오는 변수 타입에 맞춰서 값을 입력해야 하는데 실수로 정수가 아닌
		//문자를 입력할 경우 에러가 발생한다. 그렇기 때문에 예외처리로 에러가 
		//발생했는지를 여기에서 체크하여 에러가 발생하면cin 내부의 에러버퍼를
		...
		if (cin.fail())
		{
			...
			//바이트 정도 지정해주었다. 2번째는 찾고자 하는 문자를 넣어준다.
		...
		if (iMenu == MENU_EXIT)
			break; //종료메뉴를 switch구문에 넣는다면 while문을 빠져나오기
				   //쉽지않다. 따라서 switch구문으로 들어가기전에 예외처리해준다.
